...
-- This view exposes city, area, and locality directly on flats rows
...
-- Fix get_locality_rankings — now joins correctly, removes invalid seeker_pins references
...
BEGIN
  ...
  SELECT
    ...
    COUNT(f.id)::NUMERIC as demand_score
  FROM flats f
  JOIN floors fl ON f.floor_id = fl.id
  JOIN buildings b ON fl.building_id = b.id
  WHERE b.city = p_city AND f.rent_amount > 0
